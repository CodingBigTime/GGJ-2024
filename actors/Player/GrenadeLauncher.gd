...
const GRENADE_SCENE := preload("res://actors/Player/Grenade.tscn")
...
func _update_throw_velocity() -> void:
	...
	# var camera_forward_distance := camera.global_position.project(throw_direction).distance_to(
	# _launch_point.global_position.project(throw_direction)
	# )
...
func _draw_throw_path() -> void:
	...
	while time_current < end_time:
		...
		var point_current := (
			_throw_velocity * time_current
			+ Vector3.DOWN * gravity * 0.5 * time_current * time_current
		)
