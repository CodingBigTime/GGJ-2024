...
enum WeaponType { DEFAULT, GRENADE }
...
@onready
var _attack_animation_player: AnimationPlayer = $CharacterRotationRoot/MeleeAnchor/AnimationPlayer
...
@onready var _equipped_weapon: WeaponType = WeaponType.DEFAULT
...
func _ready() -> void:
	...
	emit_signal("weapon_switched", WeaponType.keys()[0])
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("swap_weapons"):
		_equipped_weapon = (
			WeaponType.DEFAULT if _equipped_weapon == WeaponType.GRENADE else WeaponType.GRENADE
		)
		_grenade_aim_controller.visible = _equipped_weapon == WeaponType.GRENADE
		emit_signal("weapon_switched", WeaponType.keys()[_equipped_weapon])

	# Get input and movement state
	var is_attacking := (
		Input.is_action_pressed("attack") and not _attack_animation_player.is_playing()
	)
	...
	if is_aiming:
		_last_strong_direction = (
			(_camera_controller.global_transform.basis * Vector3.BACK).normalized()
		)
	...
	if is_aiming:
		...
		_grenade_aim_controller.throw_direction = (
			_camera_controller.camera.quaternion * Vector3.FORWARD
		)
...
func _orient_character_to_direction(direction: Vector3, delta: float) -> void:
	...
	_rotation_root.transform.basis = (
		Basis(
