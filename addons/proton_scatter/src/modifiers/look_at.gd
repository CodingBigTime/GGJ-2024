...
func _init() -> void:
	...
	(
		documentation
		. add_paragraph(
			"Rotates every transform such that the forward axis (-Z) points towards the target position."
		)
	)
	...
	documentation.add_parameter("Up").set_type("Vector3").set_description("Up axes (X, Y, Z)")
