...
func set_handle(
	...
	var points = Geometry3D.get_closest_points_between_segments(
		origin, (Vector3.LEFT * 4096) * gt_inverse, ray_from, ray_to
	)
...
func redraw(plugin: EditorNode3DGizmoPlugin, gizmo: EditorNode3DGizmo):
	...
	if is_selected(gizmo):
		for i in steps:
			...
			lines.append(
				Vector3(cos((i + 1) * step_angle), sin((i + 1) * step_angle), 0.0) * radius
			)

		for i in steps:
			lines.append(Vector3(0.0, cos(i * step_angle), sin(i * step_angle)) * radius)
			lines.append(
				Vector3(0.0, cos((i + 1) * step_angle), sin((i + 1) * step_angle)) * radius
			)
