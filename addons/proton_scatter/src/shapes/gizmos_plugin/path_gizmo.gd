...
func set_handle(
	...
	var click_world_position := _intersect_with(
		shape_node, camera, screen_pos, previous_handle_position
	)
	var point_local_position: Vector3 = (
		shape_node.get_global_transform().affine_inverse() * click_world_position
	)
		...
		if handle_id % 2 == 0:
			...
			if mirror_angle:
				if not mirror_length:
					mirror_position = (
						curve.get_point_out(curve_index).length() * -in_out_position.normalized()
					)
			...
			if mirror_angle:
				if not mirror_length:
					mirror_position = (
						curve.get_point_in(curve_index).length() * -in_out_position.normalized()
					)
...
func redraw(plugin: EditorNode3DGizmoPlugin, gizmo: EditorNode3DGizmo):
	...
	gizmo.add_handles(
		in_out_handles, plugin.get_material("secondary_handle", gizmo), ids, false, true
	)
...
func _intersect_with(
	path: ProtonScatterShape, camera: Camera3D, screen_point: Vector2, handle_position_local = null
) -> Vector3:
	# Get the ray data
