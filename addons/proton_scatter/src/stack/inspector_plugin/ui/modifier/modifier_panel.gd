...
func create_ui_for(modifier) -> void:
	...
	_transform_space_ui.toggle_option_item(
		2, modifier.individual_instances_reference_frame_available
	)
	if (
		not modifier.global_reference_frame_available
		and not modifier.local_reference_frame_available
		and not modifier.individual_instances_reference_frame_available
	):
		_transform_space_ui.visible = false

	# Setup header connections
	_override_ui.value_changed.connect(
		_on_parameter_value_changed.bind("override_global_seed", _override_ui)
	)
	_custom_seed_ui.value_changed.connect(
		_on_parameter_value_changed.bind("custom_seed", _custom_seed_ui)
	)
	_restrict_height_ui.value_changed.connect(
		_on_parameter_value_changed.bind("restrict_height", _restrict_height_ui)
	)
	_transform_space_ui.value_changed.connect(
		_on_parameter_value_changed.bind("reference_frame", _transform_space_ui)
	)
	...
	for property in modifier.get_property_list():
		...
		if parameter_ui:
			...
			parameter_ui.value_changed.connect(
				_on_parameter_value_changed.bind(property.name, parameter_ui)
			)
