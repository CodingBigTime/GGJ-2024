	...
	if event is InputEventKey:
		...
		if _dragging:
			...
			if _selected_tangent == -1:  # Drag point
				var point_pos: Vector2 = _to_curve_space(event.position).snapped(
					Vector2(snap_threshold, snap_threshold * curve_amplitude)
				)
				...
				var control_pos: Vector2 = _to_curve_space(event.position).snapped(
					Vector2(snap_threshold, snap_threshold * curve_amplitude)
				)
				...
				if _selected_tangent == 0:
					...
					if (
						link
						and _selected_point != (curve.get_point_count() - 1)
						and curve.get_point_right_mode(_selected_point) != Curve.TANGENT_LINEAR
					):
					...
					if (
						link
						and _selected_point != 0
						and curve.get_point_left_mode(_selected_point) != Curve.TANGENT_LINEAR
					):
						curve.set_point_left_tangent(_selected_point, tangent)
			queue_redraw()
		else:
...
func _draw() -> void:
	...
	for i in columns + 1:
		...
		draw_string(
			_font,
			Vector2(x + margin, min_outer.y - margin),
			str(snapped(i * x_offset, 0.01)),
			0,
			-1,
			-1,
			text_color
	...
	for i in rows + 1:
		...
		draw_string(
			_font,
			Vector2(min_inner.x + margin, y - margin),
			str(snapped(y_value, 0.01)),
			0,
			-1,
			-1,
			text_color
	...
	for i in steps - 1:
		...
		draw_line(
			Vector2(min_inner.x + x_offset * i, a_y),
			Vector2(min_inner.x + x_offset * (i + 1), b_y),
			curve_color
		)
		a_y = b_y
